...
void AICarbon::_move(int xp, int yp, bool updateHash)
{
  ...
  if(xp - 2 < upperLeftCand.x) upperLeftCand.x = std::max(xp - 2, 4);
  if(yp - 2 < upperLeftCand.y) upperLeftCand.y = std::max(yp - 2, 4);
  if(xp + 2 > lowerRightCand.x) lowerRightCand.x = std::min(xp + 2, boardWidth + 3);
  if(yp + 2 > lowerRightCand.y) lowerRightCand.y = std::min(yp + 2, boardHeight + 3);
